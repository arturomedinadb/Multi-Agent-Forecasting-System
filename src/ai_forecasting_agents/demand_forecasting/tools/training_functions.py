...
async def create_model_configs(
    ...
    try:
        ...
        all_hyperparameters = (
            json.loads(hyperparameters_json) if hyperparameters_json else {}
        )
    ...
    try:
        ...
        for col in id_column:
            if col in original_id_data:
                ...
                original_id_train[col] = (
                    original_id_data[col].iloc[train_absolute_idx].values
                )
                original_id_val[col] = (
                    original_id_data[col].iloc[val_absolute_idx].values
                )
                original_id_test[col] = original_id_data[col].iloc[test_idx].values

        print(
            f"Split sizes - Train: {len(X_train)} ({len(X_train)/total_size*100:.1f}%), Val: {len(X_val)} ({len(X_val)/total_size*100:.1f}%), Test: {len(X_test)} ({len(X_test)/total_size*100:.1f}%)"
        )
        print(
            f"TimeSeriesSplit - Train: {len(train_idx)}, Val: {len(val_idx)}, Test: {len(test_idx)}"
        )
        ...
        pd.DataFrame(X_val_scaled, columns=X.columns).to_csv(
            f"{data_dir}/X_val.csv", index=False
        )
        ...
        for col in id_column:
            if col in original_id_test:
                pd.DataFrame({col: original_id_test[col]}).to_csv(
                    f"{data_dir}/id_test_{col}.csv", index=False
                )
                pd.DataFrame({col: original_id_train[col]}).to_csv(
                    f"{data_dir}/id_train_{col}.csv", index=False
                )
                pd.DataFrame({col: original_id_val[col]}).to_csv(
                    f"{data_dir}/id_val_{col}.csv", index=False
                )
...
@function_tool(
    strict_mode=False
)  # AllModelConfigs.hyperparameters is a genuinely open dict
async def train_models(
    all_model_configs: AllModelConfigs, data_dir: str, output_dir: str
) -> Dict[str, Any]:
    ...
    try:
        ...
        for config in model_configs:
            try:
                model_result = await train_single_model(
                    config, X_train, X_val, y_train, y_val, output_dir, session_uuid
                )
                ...
                if (
                    ...
                    and model_result["rmse"] < results["best_score"]
                ):
                ...
                results["models"].append(
                    {"success": False, "model_name": config.model_name, "error": str(e)}
                )
...
@function_tool(
    strict_mode=False
)  # HyperparameterTuningParameters.hyperparameters is a genuinely open dict
async def apply_hyperparameter_tuning(
    ...
    try:
        ...
        if optimization_method == "grid_search" and param_grid:
            ...
            print(
                "No hyperparameter grid provided or method not grid_search, using default parameters"
            )
        ...
        model_timestamp = (
            target_model.split("_")[-1]
            if "_" in target_model
        ...
        additional_metadata = {
            ...
            "best_cv_score": (
                best_score if optimization_method == "grid_search" else None
            ),
        }
        save_model_config(
            model_dir=model_dir,
