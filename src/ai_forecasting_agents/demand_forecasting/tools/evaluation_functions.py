...
def calculate_overall_score(rmse: float, r2: float, mae: float, mape: float) -> float:
    ...
    rmse_score = max(
        0, 100 * (1 - min(rmse / 100, 1))
    )  # Assuming max reasonable RMSE is 100
    ...
    mae_score = (
        max(0, 100 * (1 - min(mae / 50, 1))) if mae > 0 else 100
    )  # Assuming max reasonable MAE is 50

    # Normalize MAPE (lower is better)
    mape_score = (
        max(0, 100 * (1 - min(mape / 100, 1))) if mape > 0 else 100
    )  # Assuming max reasonable MAPE is 100%
...
async def evaluate_model_performance(
    model_info: TrainedModel, data_dir: str, iteration: int
) -> Dict[str, Any]:
    ...
    try:
        ...
        print(
            f"Evaluation results: RMSE: {rmse}, R2: {r2}, MAE: {mae}, MAPE: {mape}, Overall Score: {overall_score}"
        )
...
@function_tool(
    strict_mode=False
)  # TrainingResultsOutput nests TrainedModel.hyperparameters: Dict[str, Any]
async def evaluate_all_models(
    training_results: TrainingResultsOutput, iteration: int
) -> EvaluationResults:
    """
        ...
        for model_info in training_results.models:
            if model_info.success:
                ...
                evaluation = await evaluate_model_performance(
                    model_info=model_info, data_dir=data_dir, iteration=iteration
                )
...
@function_tool(
    strict_mode=False
)  # EvaluationResults nests ModelPerformance.hyperparameters: Dict[str, Any]
...
async def save_best_model_for_inference(
    ...
    try:
        ...
        print(
            f"{datetime.now()} - Retraining best model for inference on combined train+val data"
        )
        print(f"Data directory: {data_dir}")
        ...
        print(
            f"Combined dataset size: {len(X_train_val)} (train: {len(X_train)}, val: {len(X_val)})"
        )

        # Get model class based on model type
        model_type_str = (
            best_model_type.value
            if hasattr(best_model_type, "value")
        ...
        print(
            f"{datetime.now()} - Model training completed in {training_time:.2f} seconds"
        )
        ...
        try:
            ...
            print(
                f"{datetime.now()} - Making predictions on test set (size: {len(X_test)})"
            )
            ...
            try:
                ...
                print(
                    f"{datetime.now()} - Could not load ID columns, will try to identify them: {str(e)}"
                )
                ...
                if id_columns:
                    print(
                        f"{datetime.now()} - Identified potential ID columns: {id_columns}"
                    )
            ...
            print(
                f"{datetime.now()} - Test set metrics - RMSE: {test_rmse:.4f}, R²: {test_r2:.4f}, MAE: {test_mae:.4f}, MAPE: {test_mape:.4f}%"
            )
            ...
            for col in id_columns:
                try:
                    ...
                    if os.path.exists(id_file):
                        ...
                        print(
                            f"{datetime.now()} - Loaded original values for ID column: {col}"
                        )
                    else:
                        ...
                        if col in X_test.columns:
                            ...
                            print(
                                f"{datetime.now()} - Using encoded values for ID column: {col} (original not found)"
                            )
                except Exception as e:
            ...
            predictions_dict = {"actual": y_test, "predicted": y_pred_test}
            ...
            print(
                f"{datetime.now()} - Test predictions saved to: {test_predictions_path}"
            )
        ...
        print(
            f"{datetime.now()} - Best model retrained and saved for inference: {inference_model_path}"
        )
...
@function_tool
async def analyze_data_structure_for_feedback(input_file: str) -> Dict[str, Any]:
    """
...
async def categorize_feedback(
    evaluation_results: EvaluationResults, agent_feedback: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        target_model = (
            best_model_perf.model_type.value
            if hasattr(best_model_perf.model_type, "value")
            else str(best_model_perf.model_type)
        )
        ...
        if any(
            ...
            parameters = {"suggestions": suggestions, "target_model": target_model}
            ...
            keyword in feedback_lower
            for keyword in ["ensemble", "combine", "vote", "stack"]
        ):
            # Ensemble suggested
