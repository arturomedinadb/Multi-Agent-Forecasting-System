...
def _usable_group_by(
    df: pd.DataFrame, group_by: Optional[List[str]]
) -> Optional[List[str]]:
    ...
    if usable != group_by:
        ...
        warnings.warn(
            f"Dropping all-null group_by column(s) {dropped}; grouping on {usable or 'none (ungrouped)'}"
        )
...
def create_lag_features(df: pd.DataFrame, config: LagFeatureConfig) -> pd.DataFrame:
    ...
    if group_by:
        ...
        for lag in config.lags:
            ...
            df_result[lag_col] = df_result.groupby(group_by)[
...
def create_rolling_features(
    df: pd.DataFrame, config: RollingFeatureConfig
) -> pd.DataFrame:
    """
    ...
    for window in config.windows:
        for func in config.functions:
            ...
            if group_by:
                ...
                df_result[feature_name] = grouped.rolling(
...
def create_time_features(df: pd.DataFrame, config: TimeFeatureConfig) -> pd.DataFrame:
    ...
    for feature in config.features:
        if feature == "year":
            ...
            df_result["is_weekend"] = df_result[config.date_column].dt.dayofweek.isin(
                [5, 6]
            )
        elif feature == "is_month_start":
            df_result["is_month_start"] = df_result[
                config.date_column
            ].dt.is_month_start
        elif feature == "is_month_end":
            df_result["is_month_end"] = df_result[config.date_column].dt.is_month_end
        elif feature == "is_quarter_start":
            df_result["is_quarter_start"] = df_result[
                config.date_column
            ].dt.is_quarter_start
        elif feature == "is_quarter_end":
            df_result["is_quarter_end"] = df_result[
                config.date_column
            ].dt.is_quarter_end
...
def create_promotion_features(
    df: pd.DataFrame, config: PromotionFeatureConfig
) -> pd.DataFrame:
    """
    ...
    for feature in config.features:
        if feature == "discount_pct":
            ...
            if (
                config.orig_price_col in df_result.columns
                and config.net_price_col in df_result.columns
            ):
                df_result["discount_pct"] = (
                    (df_result[config.orig_price_col] - df_result[config.net_price_col])
                    / df_result[config.orig_price_col]
                    * 100
                ).fillna(0)
            ...
            if (
                config.promo_start_col in df_result.columns
                and config.promo_end_col in df_result.columns
            ):
                start_dates = pd.to_datetime(
                    df_result[config.promo_start_col], errors="coerce"
                )
                end_dates = pd.to_datetime(
                    df_result[config.promo_end_col], errors="coerce"
                )
                df_result["promo_duration"] = (end_dates - start_dates).dt.days.fillna(
                    0
                )
            ...
            if (
                config.promo_end_col in df_result.columns
                and config.date_col in df_result.columns
            ):
                promo_end_dates = pd.to_datetime(
                    df_result[config.promo_end_col], errors="coerce"
                )
                current_dates = pd.to_datetime(
                    df_result[config.date_col], errors="coerce"
                )
                df_result["days_since_promo"] = (
                    current_dates - promo_end_dates
                ).dt.days.fillna(999)

        elif feature == "promo_intensity":
            # Promotion intensity based on discount percentage and duration
            if (
                "discount_pct" in df_result.columns
                and "promo_duration" in df_result.columns
            ):
                df_result["promo_intensity"] = (
                    df_result["discount_pct"] * df_result["promo_duration"] / 100
                )
...
def create_holiday_features(
    df: pd.DataFrame, config: HolidayFeatureConfig
) -> pd.DataFrame:
    """
    ...
    for feature in config.features:
        if feature == "days_to_holiday":
            ...
            df_result["days_to_holiday"] = (
                0  # Placeholder - would need holiday calendar
            )
            warnings.warn(
                "days_to_holiday feature requires holiday calendar implementation"
            )

        elif feature == "days_from_holiday":
            # Days from last holiday
            df_result["days_from_holiday"] = (
                0  # Placeholder - would need holiday calendar
            )
            warnings.warn(
                "days_from_holiday feature requires holiday calendar implementation"
            )

        elif feature == "holiday_type":
            # Categorize holiday types
            if config.holiday_name_col in df_result.columns:
                df_result["holiday_type"] = df_result[config.holiday_name_col].fillna(
                    "No Holiday"
                )
            ...
            if config.is_holiday_col in df_result.columns:
                df_result["is_pre_holiday"] = (
                    df_result[config.is_holiday_col].shift(-1).fillna(False)
                )

        elif feature == "is_post_holiday":
            # Is day after holiday
            if config.is_holiday_col in df_result.columns:
                df_result["is_post_holiday"] = (
                    df_result[config.is_holiday_col].shift(1).fillna(False)
                )
...
def create_weather_features(
    df: pd.DataFrame, config: WeatherFeatureConfig
) -> pd.DataFrame:
    """
    ...
    for feature in config.features:
        if feature == "temp_category":
            ...
            if (
                config.temp_col in df_result.columns
                and config.precip_col in df_result.columns
            ):
                # Normalize temperature and precipitation to 0-1 scale
                temp_norm = (
                    df_result[config.temp_col] - df_result[config.temp_col].min()
                ) / (
                    df_result[config.temp_col].max() - df_result[config.temp_col].min()
                )
                precip_norm = (
                    df_result[config.precip_col] / df_result[config.precip_col].max()
                )
            ...
            if (
                ...
                temp_comfort = (
                    1 - abs(df_result[config.temp_col] - 22.5) / 50
                )  # Normalize around 22.5°C
                precip_comfort = 1 - (
                    df_result[config.precip_col] / df_result[config.precip_col].max()
                )
...
def create_economic_features(
    df: pd.DataFrame, config: EconomicFeatureConfig
) -> pd.DataFrame:
    """
    ...
    for feature in config.features:
        if feature == "cpi_change":
            ...
            if config.cpi_col in df_result.columns:
                cpi_norm = 1 - (
                    df_result[config.cpi_col] - df_result[config.cpi_col].min()
                ) / (df_result[config.cpi_col].max() - df_result[config.cpi_col].min())
                health_components.append(cpi_norm)
            if config.gdp_col in df_result.columns:
                gdp_norm = (
                    df_result[config.gdp_col] - df_result[config.gdp_col].min()
                ) / (df_result[config.gdp_col].max() - df_result[config.gdp_col].min())
                health_components.append(gdp_norm)
            if config.unemployment_col in df_result.columns:
                unemp_norm = 1 - (
                    df_result[config.unemployment_col]
                    - df_result[config.unemployment_col].min()
                ) / (
                    df_result[config.unemployment_col].max()
                    - df_result[config.unemployment_col].min()
                )
                health_components.append(unemp_norm)

            if health_components:
                df_result["economic_health_index"] = pd.concat(
                    health_components, axis=1
                ).mean(axis=1)
...
def create_polynomial_features(
    df: pd.DataFrame, numeric_columns: List[str], degree: int = 2
) -> pd.DataFrame:
    """
...
@function_tool
def process_feature_engineering_pipeline(
    input_file: str, output_file: str, config: FeatureEngineeringConfig
) -> FeatureEngineeringResult:
    """
    ...
    try:
        ...
        if config.interaction_features:
            ...
            interaction_pairs = [
                (var1, var2) for var1 in key_vars for var2 in key_vars if var1 != var2
            ]
        ...
        if config.polynomial_features:
            ...
            df_result = create_polynomial_features(
                df_result, numeric_cols, config.polynomial_degree
            )
        ...
        if config.normalize_features:
            ...
            df_result[numeric_cols] = (
                df_result[numeric_cols] - df_result[numeric_cols].mean()
            ) / df_result[numeric_cols].std()
        ...
        feature_set = FeatureSet(
            ...
            feature_types={
                feature: FeatureType.LAG for feature in new_features
            },  # Simplified
            feature_descriptions={
                feature: f"Engineered feature: {feature}" for feature in new_features
            },
        )
