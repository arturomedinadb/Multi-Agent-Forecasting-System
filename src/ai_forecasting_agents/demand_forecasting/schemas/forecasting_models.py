...
class ModelConfig(BaseModel):
    ...
    model_uuid: str = Field(
        ...,
        description="Timestamp-based unique identifier for the model (YYYYMMDDHHMMSS)",
    )
...
class AllModelConfigs(BaseModel):
    ...
    session_uuid: str = Field(
        ...,
        description="Timestamp-based identifier for the training session (YYYYMMDDHHMMSS)",
    )
...
class TrainedModel(BaseModel):
    ...
    hyperparameters: Dict[str, Any] = Field(
        default_factory=dict, description="Hyperparameters used for training"
    )
    success: bool = Field(..., description="Whether training was successful")
    validation_rmse: Optional[float] = Field(
        default=None, description="Validation RMSE if available"
    )
...
class TrainingResultsOutput(BaseModel):
    ...
    models_directory: str = Field(
        ..., description="Directory containing all trained model files"
    )
    data_dir: str = Field(
        ..., description="Directory containing preprocessed data splits"
    )
    ...
    session_uuid: str = Field(
        default=None,
        description="Timestamp-based identifier for training session (YYYYMMDD_HHMMSS)",
    )
    training_timestamp: Optional[str] = Field(
        default=None, description="Timestamp when training completed"
    )
...
class EvaluationResults(BaseModel):
    ...
    success: bool = Field(..., description="Whether training was successful")
    model_evaluations: List[ModelPerformance] = Field(
        ..., description="List of all model evaluations"
    )
    ...
    iteration: int = Field(..., description="Current iteration number")
    models_directory: str = Field(
        ..., description="Directory containing all model files"
    )
...
class EvaluationFeedback(BaseModel):
    ...
    parameters: Dict[str, Any] = Field(
        ..., description="Parameters for the improvement action"
    )
