...
class DataAnalysisResult(BaseModel):
    ...
    input_file_path: str = Field(
        ..., description="Path to the input file used for analysis"
    )
    ...
    boolean_columns: List[str] = Field(
        default_factory=list, description="List of boolean columns"
    )
    memory_usage_bytes: int = Field(
        ..., description="Total memory usage of the dataset in bytes"
    )
...
class RollingFeatureConfig(BaseModel):
    ...
    windows: List[int] = Field(
        default=[7, 14, 30, 90], description="Rolling window sizes"
    )
    functions: List[str] = Field(
        default=["mean", "std", "min", "max"], description="Aggregation functions"
    )
    group_by: Optional[List[str]] = Field(
    ...
    def validate_functions(cls, v):
        ...
        for func in v:
            if func not in valid_functions:
                raise ValueError(
                    f"Function '{func}' not supported. Valid functions: {valid_functions}"
                )
        return v
...
class TimeFeatureConfig(BaseModel):
    ...
    date_column: str = Field(
        default="transaction_date", description="Date column to extract features from"
    )
    ...
    def validate_features(cls, v):
        ...
        for feature in v:
            if feature not in valid_features:
                raise ValueError(
                    f"Feature '{feature}' not supported. Valid features: {valid_features}"
                )
        return v
...
class PromotionFeatureConfig(BaseModel):
    ...
    promo_active_col: str = Field(
        default="promo_active", description="Promotion active column"
    )
    promo_price_col: str = Field(
        default="promo_price", description="Promotion price column"
    )
    orig_price_col: str = Field(
        default="unit_orig_price", description="Original price column"
    )
    net_price_col: str = Field(default="unit_net_price", description="Net price column")
    promo_start_col: str = Field(
        default="promo_start_date", description="Promotion start date column"
    )
    promo_end_col: str = Field(
        default="promo_end_date", description="Promotion end date column"
    )
    date_col: str = Field(
        default="transaction_date", description="Date column, used for days_since_promo"
    )
...
class HolidayFeatureConfig(BaseModel):
    ...
    holiday_name_col: str = Field(
        default="holiday_name", description="Holiday name column"
    )
    is_holiday_col: str = Field(
        default="is_holiday", description="Is holiday boolean column"
    )
...
class WeatherFeatureConfig(BaseModel):
    ...
    precip_col: str = Field(
        default="precipitation_mm", description="Precipitation column"
    )
...
class EconomicFeatureConfig(BaseModel):
    ...
    unemployment_col: str = Field(
        default="unemployment_rate_monthly", description="Unemployment rate column"
    )
    population_col: str = Field(
        default="population_monthly", description="Population column"
    )
...
class FeatureEngineeringConfig(BaseModel):
    ...
    interaction_features: bool = Field(
        default=True, description="Create interaction features between key variables"
    )
    polynomial_features: bool = Field(
        default=False, description="Create polynomial features for numeric variables"
    )
    polynomial_degree: int = Field(
        default=2, description="Degree for polynomial features"
    )

    # Data processing options
    handle_missing: str = Field(
        default="forward_fill", description="Strategy for handling missing values"
    )
    ...
    def validate_handle_missing(cls, v):
        ...
        if v not in valid_strategies:
            raise ValueError(
                f"Invalid missing value strategy. Valid options: {valid_strategies}"
            )
        return v
...
class FeatureSet(BaseModel):
    ...
    feature_types: Dict[str, FeatureType] = Field(
        ..., description="Mapping of feature names to types"
    )
    feature_descriptions: Dict[str, str] = Field(
        default_factory=dict, description="Descriptions of each feature"
    )
    created_at: datetime = Field(
        default_factory=datetime.now, description="When features were created"
    )

    def get_features_by_type(self, feature_type: FeatureType) -> List[str]:
        """Get all features of a specific type."""
        return [
            name for name, ftype in self.feature_types.items() if ftype == feature_type
        ]
...
class FeatureEngineeringResult(BaseModel):
    ...
    def feature_ratio(self) -> float:
        ...
        return (
            self.features_created / original_features if original_features > 0 else 0.0
        )
