...
async def main():
    ...
    if not mapped_data_file.exists():
        ...
        print(
            "   Please ensure the mapping_target.csv file exists in the data directory"
        )
        return 1
    ...
    feature_output_file = (
        str(Path(__file__).parent.parent / "output" / "feature_engineered_output.csv"),
    )
    ...
    with trace("Feature Engineering", group_id=conversation_id):
        feature_engineering_result = await Runner.run(
    ...
    training_output_dir = str(
        Path(__file__).parent.parent / "output" / "training_results"
    )
    inference_dir = str(
        Path(__file__).parent.parent / "output" / "training_results" / "inference"
    )
    ...
    with trace("Demand Forecasting Training", group_id=conversation_id):
        training_result = await Runner.run(
            training_agent, input=initial_message, max_turns=150, session=session
        )
