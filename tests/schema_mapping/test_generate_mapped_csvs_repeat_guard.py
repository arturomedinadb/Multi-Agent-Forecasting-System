...
class TestGenerateMappedCsvsRepeatGuard:
    def test_second_call_returns_cached_result_even_with_malformed_arguments(
        ...
        second = _invoke(
            source_metadata_json="not valid json {{{",
            mappings_json="also not valid",
            output_dir=str(tmp_path),
        )
        assert second == first
