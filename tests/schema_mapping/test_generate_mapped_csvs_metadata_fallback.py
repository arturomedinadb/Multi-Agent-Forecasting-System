...
unparseable, or incomplete - real failure modes when the model has to
re-echo a large metadata payload as a literal argument and either truncates
it or only includes a subset of the files.
"""
import json
import sqlite3
...
class TestGenerateMappedCsvsMetadataFallback:
    def test_recovers_from_session_db_when_source_metadata_json_is_truncated(self, tmp_path, monkeypatch):
        ...
        mapping_plan = json.dumps({
            "mappings": [{"source_file": file_path, "mappings": [
        ...
        result = json.loads(run_generate_mapped_csvs(truncated_metadata_json, mapping_plan, str(tmp_path)))

        assert result.get("error") is None
        assert len(result["outputs"]) == 1
        assert result["outputs"][0]["columns"] == ["date"]

    def test_recovers_from_session_db_when_source_metadata_json_is_empty(self, tmp_path, monkeypatch):
        src_path = tmp_path / "x.csv"
        pd.DataFrame({"a": [1, 2]}).to_csv(src_path, index=False)
        file_path = str(src_path).replace("\\", "/")
    ...
    def test_uses_session_db_when_argument_only_covers_a_subset_of_files(self, tmp_path, monkeypatch):
        """A real failure seen in practice: the model supplies a
        source_metadata_json that parses fine but only contains 1 of the
        actual N files loaded this session. The DB's more complete record
        must win instead of silently mapping only that one file."""
        ...
        # Only mentions first_path, mirroring an LLM that dropped a file.
        partial_metadata_json = json.dumps([{"file_path": first_path, "columns": ["a"]}])
        mapping_plan = json.dumps({
            "mappings": [
                {"source_file": first_path, "mappings": [
                    {"source_column": "a", "target_column": "date", "confidence": 0.9, "reasoning": "r"}
                ]},
                {"source_file": second_path, "mappings": [
                    {"source_column": "b", "target_column": "product_id", "confidence": 0.9, "reasoning": "r"}
                ]},
            ]
        })

        result = json.loads(run_generate_mapped_csvs(partial_metadata_json, mapping_plan, str(tmp_path)))
