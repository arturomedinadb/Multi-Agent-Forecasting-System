...
class TestGetAllDatasetMetadataRetry:
    def test_returns_immediately_when_all_results_already_present(
    ...
    def test_waits_for_a_result_that_lands_late(self, tmp_path, monkeypatch):
        """When the last call's result hasn't been persisted yet, the retry
        picks it up rather than returning a short list."""
        db_path = tmp_path / "workflow_sessions.db"
    ...
    def test_gives_up_gracefully_when_a_result_never_arrives(
        self, tmp_path, monkeypatch
    ):
